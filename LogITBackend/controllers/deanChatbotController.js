...
const buildStudentSummary = ({ student, approvedHours, logsCount, evaluation }) => {
...
const resolveStudentSummary = async (query) => {
	...
	if (!isAdmissionLookup && students.length > 1) {
		return [
			...
			...students
				.slice(0, 5)
				.map((student) => `• ${student.name} (${student.student_admission_number || "N/A"})`),
...
const resolveCompanySummary = async (query) => {
	...
	const completedStudents = enrichedStudents.filter(
		(student) => Number(student.progress || 0) >= 100 || student.completed_program,
	).length;

	const stats = {
		totalStudents: enrichedStudents.length,
		completedStudents,
		inProgressStudents: Math.max(0, enrichedStudents.length - completedStudents),
		totalHoursLogged,
